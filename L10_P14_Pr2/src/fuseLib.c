...
static int my_unlink(const char *path){
    ...
    // Update root folder
    myFileSystem.directory.files[idxDir].freeFile = true;//Marcamos el fichero como libre
    myFileSystem.directory.numFiles--;//Restamos el numero de ficheros en el sistema
    //strcpy(myFileSystem.directory.files[idxDir].fileName, path + 1);
    idxNodoI =  myFileSystem.directory.files[idxDir].nodeIdx;//Sacamos el numero de i-nodo que queremos borrar
    myFileSystem.numFreeNodes++;//Sumamos el i-nodo libre a la lista de i-nodos libres

    // Fill the fields of the new inode
   

    //myFileSystem.nodes[idxNodoI]->fileSize = 0; innecesaria
    //myFileSystem.nodes[idxNodoI]->numBlocks = 0; innecesaria
    //myFileSystem.nodes[idxNodoI]->modificationTime = time(NULL); innecesaria
    myFileSystem.nodes[idxNodoI]->freeNode = true;//Marcamos el inodo como no ocupado
     
   
    resizeNode(idxNodoI, 0);//Ponemos el tamaño del i-nodo a 0
    
   
    free(myFileSystem.nodes[idxNodoI]);//Liberamos todo el espacio ocupado por el i-nodo
    
    //reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNodoI]->blocks, 0);
   
    updateDirectory(&myFileSystem);//Actualizamos el sistema
    updateNode(&myFileSystem, idxNodoI, myFileSystem.nodes[idxNodoI]);//Actualizamos el i-nodo en el sistema
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
    ...
    NodeStruct *node = myFileSystem.nodes[fi->fh];
    int idxDir = -1;
    int bytes2Read = min(node->fileSize,size), totalRead = 0;
  ...
  //  if(resizeNode(fi->fh, size + offset) < 0)
    ...
    while(bytes2Read > 0) {
        ...
        if( readBlock(&myFileSystem, currentBlock, &buffer)==-1 ) {//Leemos lo equivalente a un bloque de informacion
